        ...
        <div data-spy="scroll" data-target="#list-example" data-offset="0" class="scrollspy-example col-10">
            <table class="table caption-top" id="tableCustomer">
                ...
                <tr>
                    ...
                    <th scope="col">Date Create Contract</th>
                ...
                <c:forEach items="${contracts}" var="contract">
                    <tr>
                        <th scope="col">${contract.getId()}</th>
                        <td>${contract.getCustomer().getName()}</td>
                        <td>${contract.getEmployee().getName()}</td>
                        <td>${contract.getService().getId()}</td>
                        <td>${contract.getDateCreateContract()}</td>
                        <td>${contract.getDateEnd()}</td>
                        <td>${contract.getDeposits()}</td>
                        <td>${contract.getTotalMoney()}</td>
                        <td>
                            <!-- Button trigger modal -->
                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampleModal-${contract.getId()}">
                                Delete
                            </button>
                            <!-- Modal -->
                            <div class="modal fade" id="exampleModal-${contract.getId()}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            Bạn có chắc muốn xóa Customer có ID: ${contract.id} của Khách Hàng ${contract.getCustomer().getName()}
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                            <button type="button" class="btn btn-danger" onclick="window.location.href='/contract?actionUser=delete&id=${contract.id}'">Xác nhận</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <tr>
                    <td>
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal" onclick="window.location.href='/contract?actionUser=create'">Thêm khách hàng</button>
                    </td>
                </tr>
            </table>
        </div>
